...
| Buyer / End User | Places orders, tracks shipments |
| Customer Success | Provisions tenants, resets passwords, audits activity |
| Finance | Reconciles orders, refunds, invoices |
...
- **Catalog** — product CRUD, category management, low-stock signalling
...
| Item | Reason |
|---|---|
| Payments / billing (real gateway) | Stub implementation wired; real PCI-scope gateway deferred to a billing service |
...
| FR-OR-5 | Order totals (subtotal, tax, shipping, total) are computed by the domain, not the client; tax rate and shipping cost are applied before confirmation |
| FR-OR-6 | Order confirmed, shipped, and fulfilled emit integration events consumed asynchronously by the Notification worker |
...
| FR-NT-4 | On `OrderCancelledDueToStock`, notify the buyer that the order was cancelled due to stock unavailability |
| FR-NT-5 | On `OrderCancelledDueToPayment`, notify the buyer that payment failed |
| FR-NT-6 | On `OrderCancelledDueToShipment`, notify the buyer that the shipment could not be created |
...
| FR-GW-2 | The gateway validates JWTs before forwarding to product/order/notification routes |
...
- PostgreSQL is the system of record; no plan to swap to another RDBMS
...
- Open-source-only stack; no proprietary cloud services in the core (cloud-managed PG/Redis/Rabbit are acceptable substitutes in production)
...
| Aggregate | A consistency boundary in DDD — Order, Product, User |
